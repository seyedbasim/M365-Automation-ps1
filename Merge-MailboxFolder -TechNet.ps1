...
#
# By David Barrett, Microsoft Ltd. 2015-2019. Use at your own risk.  No warranties are given.
#
#  DISCLAIMER:
...
Function IncreaseThrottlingDelay()
{
    ...
    $maxThrottlingDelay = 30000
    if ( $script:currentThrottlingDelay -ge $maxThrottlingDelay) { return }
    ...
    if ( $script:currentThrottlingDelay -lt $maxThrottlingDelay)
    {
        ...
        LogVerbose "Increased throttling delay to $($script:currentThrottlingDelay)ms"
    }
...
Function ThrottledBatchMove()
{
    ...
    $originalThrottlingDelay = $script:currentThrottlingDelay
    if ( $totalItems -gt 10000 )
    {
        if ( $script:currentThrottlingDelay -lt 10000 )
        {
            $script:currentThrottlingDelay = 10000
            LogVerbose "Large number of items will be processed, so throttling delay set to 10s"
        }
    }
    ...
    while ( !$finished )
    {
        ...
        try
        {
            if (!$WhatIf)
            {
                ...
                LogVerbose "Waiting for $($script:currentThrottlingDelay)ms"
                Start-Sleep -Milliseconds $script:currentThrottlingDelay
            }
        }
        catch
        {
            ...
            else
            {
                ...
                {
                    if ($Error[0].Exception.InnerException.ToString().Contains("The operation has timed out"))
                    {
    ...
    if ($originalThrottlingDelay -ne $script:currentThrottlingDelay)
    {
        LogVerbose "Restoring throttling delay to $($originalThrottlingDelay)ms"
        $script:currentThrottlingDelay = $originalThrottlingDelay
    }
}
...
Function MoveItems()
{
	...
	while ($MoreItems)
	{
        ...
        if ($FindResults)
        {
            ...
            if ($MoreItems)
            {
                LogVerbose "[MoveItems]$($itemsToMove.Count) items found so far, more available"
            }
		    $Offset += $PageSize
        }
